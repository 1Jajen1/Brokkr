...
newtype FreshEntityId = FreshEntityId (MVar FreshEntityId_)
...
new = do
  ...
  FreshEntityId <$> newMVar (FreshEntityId_ 0 0 arr indArr)

allocateEntityId :: FreshEntityId -> IO EntityId
allocateEntityId (FreshEntityId ref) = do
  FreshEntityId_ sz highestId arr indArr <- takeMVar ref
  ...
  if sz == highestId
    then do
      ...
      putMVar ref $ FreshEntityId_ (sz + 1) (highestId + 1) arr' indArr'
      -- return the newly generated id
      pure $ EntityId highestId
    else do
      ...
      -- increment the size
      putMVar ref $ FreshEntityId_ (sz + 1) highestId arr indArr
      -- return the reused id
      pure $ EntityId newId

deAllocateEntityId :: FreshEntityId -> EntityId -> IO ()
deAllocateEntityId (FreshEntityId ref) (EntityId eid) = do
  old@(FreshEntityId_ sz highestId arr indArr) <- takeMVar ref
...
data FreshEntityId_ = FreshEntityId_
