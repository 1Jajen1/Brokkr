...
static __inline__ __m256i check_valid_block_256(const __m256i inp, const __m256i prevInput) {
  ...
  return _mm256_or_si256(inValidSurrogates, _mm256_xor_si256(valid3Cont, twos));
}
...
__attribute__((target("avx2")))
static inline int is_valid_cesu8_m256(uint8_t const *const src, size_t len) {
...
static __inline__ __m128i check_valid_block_128(const __m128i inp, const __m128i prevInput) {
  ...
  return _mm_or_si128(inValidSurrogates, _mm_xor_si128(valid3Cont, twos));
}
...
__attribute__((target("sse4.2")))
static inline int is_valid_cesu8_m128(uint8_t const *const src, size_t len) {
...
int is_valid_cesu8(uint8_t const *const src, size_t len) {
  ...
  if (unlikely(len >= 64)) {
    if (__builtin_cpu_supports("avx2")) {
      return is_valid_cesu8_m256(src, len);
    } else {
      return is_valid_cesu8_m128(src, len);
    }
  } else {
