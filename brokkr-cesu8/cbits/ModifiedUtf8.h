  ...
  while (likely(ptr < end)) {
    ...
    if (likely(byte > 0 && byte < 0b10000000)) {
      ...
      if (likely(ptr + 8 < end && true)) {
        ...
        uint64_t noNull = ~((word + low_bits_mask) | word | low_bits_mask);
        if (validAscii & noNull == 0) {
          ptr += 8;
        } else {
          uint64_t firstNonAscii = __builtin_ctzl(validAscii) >> 3;
          if (noNull == 0) { // Don't accidentally skip null bytes too
            // So instead we need to look for null bytes first, high bits second
            // printf("Some %i %i %i %i\n", byte, *ptr, tmp, firstNonAscii);
            ptr += firstNonAscii;
          } else {
            // printf("Invalid null");
            return 0;
