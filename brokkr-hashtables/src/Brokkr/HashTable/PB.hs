...
lookup :: (PrimMonad m, Eq key, Hash key, Prim key) => HashTable (PrimState m) key value -> key -> m (Maybe value)
{-# INLINE lookup #-}
lookup ht key = lookupWithHash ht key (coerce (hash key) (hashSalt ht))

lookupWithHash :: forall key value m . (PrimMonad m, Eq key, Prim key) => HashTable (PrimState m) key value -> key -> Int -> m (Maybe value)
{-# INLINE lookupWithHash #-}
lookupWithHash HashTable_PB{..} key !hs = do
